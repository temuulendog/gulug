...
public class DataReader {
        ...
        socket.emit("send_file", data, new Ack() {
            ...
            public void call(Object... os) {
                    ...
                    if(os.length > 0){
                        ...
                        if (action){
                            //fileID generated by server then return with this function
                            fileID = (int) os[1];
                            //starting send file
                            try {
                                sendingFile(socket);
    ...
    private void sendingFile(Socket socket) throws  IOException, JSONException{
        ...
        socket.emit("sending", data, new Ack() {
            @Override
            public void call(Object... os) {
                //call back function to sending more files
                //this function meaning the server has recived file we has sending
                //so we need send more file until we finish
                //we responce boolean true or false
                if(os.length > 0){
                    boolean act = (boolean) os[0];
                    if (act){
                        try {
                            ////this function will recursiveuntil act = false
